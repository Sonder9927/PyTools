...
# download video
def down(name, url, dir_file: Path):
    headers_down = {
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3947.100 Safari/537.36",
    }
    r = requests.get(url, headers=headers_down)
    with open(dir_file / f"{str(name)}.mp4", "wb+") as f:
        f.write(r.content)
...
def getlist(sec_id, dir_file: Path):
    ...
    sec_uid = sec_id
    while has_more:
        url = f"https://www.iesdouyin.com/web/api/v2/aweme/post/?sec_uid={str(sec_uid)}&count={count}&max_cursor={str(max_cursor)}&aid=1128&_signature=z1epBAAArxEnjYt5fPWXJs9XqR&dytk="
