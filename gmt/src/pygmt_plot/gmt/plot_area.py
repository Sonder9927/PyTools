...
def _sed_moho(region):
    ...
    tomo_grid_data(sed, tomos[0]["grid"], region)
    cmap = "src/txt/cptfiles/Vc_1.8s.cpt"
    pygmt.makecpt(cmap=cmap, series=[0, 2, 0.05], output=tomos[0]["cmap"])
    moho = pd.read_csv(
        "src/txt/moho.lst",
        header=None,
        usecols=[0, 1, 2],
    ...
    moho = moho[["y", "x", "z"]]
    moho.columns = ["x", "y", "z"]
    tomo_grid_data(moho, tomos[1]["grid"], region)
    pygmt.makecpt(cmap=cmap, series=[27, 35, 0.1], output=tomos[1]["cmap"])
    return tomos
...
def lines_generator(region, linetype):
    ...
    yy = region[-2:]
    d = 0.5
    df = pd.read_csv("src/txt/area_hull.csv")
