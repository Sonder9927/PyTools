...
def dp_title_fname_tmpgrd_stdgrd(region, grid, std):
    ...
    per = ps[-3].name
    fname = f"plot/figs/grid_{per}_AS.png"
...
def dp_grid(region, grid, std, projection, VEL_CPT, TOPO_GRA):
    ...
    # fig.plot(data="plot/CN-block-L2.gmt", pen="thick,black,-")
    # plot weihe
...
def dp_AS():
    region= [115.5, 122.5, 29, 35]  # [118.5, 122.5, 29, 32.6]
    ic(region)
    ...
    # loop = Path("loop")
    # grid_list = list(loop.glob("TPWT_10_8_80_2/*/new*/grid.*"))
    # grid_list = [i for i in grid_list if i.suffix != ".ave"]
    # std_list = list(loop.glob("TPWT_10_8_80_2/*/new*/stddev.*_v2"))
    work = Path("./")
    grid_list = list(work.glob("TPW_10snr_8t_3500dist/*/new*/grid.*"))
    grid_list = [i for i in grid_list if i.suffix != ".ave"]
    std_list = list(work.glob("TPW_10snr_8t_3500dist/*/new*/stddev.*_v2"))
